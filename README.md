...
- **Vector DB (RAG):** 実際に実装済みです。`dvds` テーブルの `description`（説明文）を `sentence-transformers` (多言語モデル) を用いてベクトル化し、SQLiteベースの独自ベクトルストアに保存しています。これにより、「泣ける映画」「家族で楽しめる」といった抽象的なクエリに対し、意味的に近い映画を検索するセマンティック検索を実現しています。
