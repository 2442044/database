...
- **状況の可視化**: 登録ユーザー数、DVD在庫数、現在の貸出中件数をリアルタイムで集計表示します。
- **履歴の即時確認**: 直近のレンタル履歴5件をトップページに表示し、最近の動向をすぐに把握できます。

![Dashboard](docs/home.png)

### 2. DVD管理（AIハイブリッド検索搭載）
在庫DVDの効率的な管理と、高度な検索機能を提供します。
- **AI検索 (セマンティック検索)**: 自然言語処理モデル（Sentence Transformers）を活用し、「週末に家族で笑えるもの」「泣ける名作」といった**感覚的なワード**から、作品の内容に基づいた検索が可能です。
- **キーワード検索とのハイブリッド**: 単純な単語一致とAIによる意味的類似度を組み合わせ、精度の高い検索結果を導き出します。
- **在庫・配置管理**: 在庫数だけでなく、店舗内の「保管場所」も記録できるため、実店舗でのピッキング作業をサポートします。

![DVD List](docs/dvds.png)

### 3. ユーザー管理
会員情報の安全な管理を行います。
- **会員コード管理**: 独自の会員番号（メンバーコード）による確実な本人特定が可能です。
- **データ整合性の保護**: 貸出履歴があるユーザーは誤って削除できないよう制限がかかっており、ビジネス上の記録漏れを防ぎます。

![User List](docs/users.png)

### 4. 貸出・返却
店舗業務の核となる処理を、最小限のステップで完結させます。
- **直感的なインターフェース**: ユーザーとDVDを選択するだけで貸出が完了。
- **ワンクリック返却**: 未返却リストから該当項目を探し、ボタン一つで返却処理と在庫の自動復元が行われます。
- **トランザクション管理**: 貸出処理と在庫の減算をセットで実行し、システムエラーによる在庫数の不整合を完全に防止します。
...
### 🚀 現場に即した実用的な設計
- **店舗運営を支える「保管場所」管理**: DVDのタイトルだけでなく、棚番号などの保管場所を記録できるため、小規模店舗での実業務を想定した作りになっています。
- **ミスを防ぐビジネスロジック**: 貸出中のDVDの削除禁止や、履歴保持ユーザーの保護など、現場でのオペレーションミスをシステム側でガードします。

### 🧠 最先端のAI技術を統合
- **「思い出せない」を助けるAI検索**: タイトルを忘れてしまった顧客に対しても、ストーリーの断片や雰囲気から作品を提案できる、次世代のレンタル管理体験を提供します。
- **多言語対応**: 多言語対応の埋め込みモデルを採用しており、日本語だけでなく幅広いクエリに対応可能です。

### 🛠️ 導入と拡張のしやすさ
- **Dockerによる即時展開**: 複雑な環境構築は不要。`docker compose up` ひとつで、データベースからリバースプロキシまで全ての環境が整います。
- **軽量・高速なSQLite採用**: サーバーのセットアップが不要なSQLiteを使用しており、小規模な環境でも高速に動作します。
- **スケーラブルな構成**: Nginxをフロントに配置しているため、将来的な同時アクセス数の増加や、バックエンドの冗長化にも対応しやすいアーキテクチャです。
