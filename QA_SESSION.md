...
**実装コード (dvd_rental_app/app.py 8-11行目付近):**
```python
# Flaskアプリケーションの初期化
app = Flask(__name__)
...
**実装コード (dvd_rental_app/init_db.py 11-16行目付近):**
```python
...
**実装コード (dvd_rental_app/init_db.py 32-43行目付近):**
```python
cursor.execute('''
...
**実装コード (dvd_rental_app/init_db.py 19-29行目付近):**
```python
# UNIQUE制約によるインデックス作成
cursor.execute('''
